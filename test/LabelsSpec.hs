...
spec = describe "Labels" $ do
      describe "labels" $ do
            ...
            it
                        "generates labels for times by aligning to a time unit (ie, days, minutes, etc)"
      ...
      describe "determineStepSize" $ do
            prop
                  ...
                  $ \(Positive minutes) (Positive start) ->
                          ...
                          in  (determineStepSize (start, end) === Minute)

            prop
                        "when there are more than 5 minutes, 5 minute increments are used"
                  . forAll (arbitrary `suchThat` (< Positive 50))
                  $ \(Positive minutes) (Positive start) ->
                          let end = start + 300 + (minutes * 60)
                          in  (determineStepSize (start, end) === FiveMinute)

            prop
                        "when there are more minutes than two hours, hour long increments are used"
                  . forAll (choose (2, 23))
                  $ \hours start ->
                          let end = start + (hours * 3600)
                          in  (determineStepSize (start, end) === Hour)

            prop
                        "when there are more minutes than two days, day long increments are used"
                  . forAll (choose (2, 7))
                  $ \days' start ->
