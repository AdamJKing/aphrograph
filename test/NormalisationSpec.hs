...
spec = describe "normalise" $ do
  ...
  prop "a range that is linearly smaller/bigger by a factor f is the same as doing ( f * that value )" $ do
    f <- factor
    (l, h) <- range
    v <- choose (l, h)
  ...
  prop
    ...
    $ do
      ...
      v <- choose target
      a <- arbitrary
      let outcome = evaluateNF (normalise (a, a) target v) `shouldThrow` anyErrorCall
      return outcome
  ...
  prop
    ...
    $ do
      ...
      let outcome = evaluateNF (normalise (lowEnd, highEnd) target value) `shouldThrow` anyErrorCall
      return outcome
