...
spec = describe "normalise" $ do
    it "rejects bad origin ranges"
        ...
        $ \(sameValue :: Double) (Range {..} :: Range Double) (value :: Double) ->
              normalise (sameValue, sameValue) (lower, higher) value === Left BadOriginRange

    it "rejects bad target ranges"
        . property
        $ \(sameValue :: Double) (Range {..} :: Range Double) (value :: Double) ->
              normalise (lower, higher) (sameValue, sameValue) value === Left BadTargetRange
    ...
    it "rejects bad values" . property $ \(Range l h) ->
        ...
        in  do
                ...
                return . counterexample (show i) $ normalise (l, h) (l', h') i === Left BadValue
    ...
    it "produces a value x times larger when the second range is x times smaller"
        . property
        $ \(Positive f :: Positive Double, Range { lower = l, higher = h }) ->
              forAll (choose (l, h)) $ \v ->
                  let outcome = normalise (l, h) (l / f, h / f) v
