...
determineStepSize (earliest, latest) | deltaDays earliest latest > 1    = Day
...
generateLabelsDiscrete :: (Show a, Integral a) => [a] -> (Int, Int) -> [(Int, Text)]
...
generateLabelsContinuous :: (Show a, RealFrac a) => [a] -> (Int, Int) -> [(Int, Text)]
